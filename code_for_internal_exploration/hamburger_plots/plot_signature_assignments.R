...
# MSI-H samples are colored red with triangle markers.
#
# Reads assignment data from Manuscript_data/finalized_cap9/ and sample info
# from Manuscript_data/sample_info.tsv.
#
# Arguments:
#   --min-fraction  Only plot a sample if the signature accounts for >= this
...
#   Rscript plot_signature_assignments.R --min-fraction 0.05
...
in_out_pairs <- list(
  list(
    input_file = "liu_et_al_83_assignment.tsv",
    output_file = "83_assignments.pdf"
  ),
  list(
    input_file = "liu_et_al_89_assignment.tsv",
...
read_assignment_and_plot_hamburger <- function(
  ...
  # Prefix column names with "CancerType::" using sample_info lookup,
  # since the finalized assignment files use bare sample IDs
  if (!is.null(sample_info)) {
