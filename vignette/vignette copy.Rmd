...
```{r loop, fig.width=19,fig.height=4, echo=FALSE, results="asis", warning = FALSE }
...
for (i in c(1:nrow(ID89_ID83_connection_example))) {
  ...
  if (ID89signature %in% c("InsDel15", "InsDel16")) {
    ...
    if (ID89signature %in% colnames(final.Koh476.signatures)) {
      cosine476 <- round(
        lsa::cosine(
          as.numeric(final.Koh476.signatures[, ID89signature]),
          as.numeric(to.plot.all.ID476.catalogs[, catalog])
        ),
        3
      )
      cat(paste0(
        "\n\n Cosine similarity of 476-Type Signature vs Examplar: ",
        cosine476,
        "\n\n"
      ))
      ...
      p6 <- plot_476(
        ...
        text_size = 3,
        plot_title = paste0(
          "476-Type Exemplar Spectrum for ",
          ID89signature,
          sep = ""
        ),
        num_labels = 5,
      ...
      grid.arrange(p6, nrow = 1, ncol = 1, heights = plot476_height)
    } else {
      cat(paste0(
        "\n\n 476-Type ",
        ID89signature,
    ...
    if (ID83signature %in% c("C_ID7", "ID_J", "C_ID10", "ID_N", "ID_O")) {
      cosine83 <- round(
        lsa::cosine(
          as.numeric(to.plot.all.signatures.final[, ID89_ID83_connection_example$ID83_signature[
            i
          ]]),
          as.numeric(to.plot.WGS_PCAWG_HMF_indels.catalog.no.polyT[, catalog])
        ),
        3
      )
      cat(paste0(
        "\n\n Cosine similarity of 83-Type Signature vs Examplar: ",
        cosine83,
        "\n\n"
      ))
      ...
      cosine83 <- round(
        lsa::cosine(
          ...
          as.numeric(to.plot.WGS_PCAWG_HMF_indels.catalog[, catalog])
        ),
        3
      )
      cat(paste0(
