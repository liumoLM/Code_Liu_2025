...
if (!is.null(plot_paths$id83_sig)) {
  if (sig_data$is_polyT_removed) {
    catfdiv(
      "In the supporting tumor
    mutations counts for insertions and deletions of T in
    long-poly-T contexts were set to 0 in the plot and
    when calculating cosine similarity between the supporting 
    tumor and the signature, but the signature was not altered 
  ...
  if (!is.null(plot_paths$id83_sig_ablated)) {
    cat(
      '\n\n<details>\n<summary>Click to see with insertions and/or deletions in long poly-T suppressed</summary>\n\n'
    )
...
if (!is.null(plot_paths$id83_mapped)) {
  ...
  if (!is.null(plot_paths$id83_mapped_ablated)) {
    cat(
      '\n\n<details>\n<summary>Insertions and/or deletions in long poly-T suppressed</summary>\n\n'
    )
