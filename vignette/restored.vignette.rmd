...
ID89.mSigAct.assignment <- read.delim(
  file.path(
    data_dir,
    "Liu_et_al_signature_assignments.tsv"
  ),
...
```{r,fig.width=19,fig.height=4, echo=FALSE, results="asis", warning = FALSE }
...
for (i in c(1:nrow(ID89_ID83_connection_example))) {
  ...
  if (ID89signature %in% c("InsDel15", "InsDel16")) {
    ...
    grid.arrange(p1, p2, nrow = 1, ncol = 2, heights = plot89_height)
    p5 <- plot_476(
    ...
    grid.arrange(p3, p4, nrow = 1, ncol = 2, heights = plot89_height)
