...
## Please set the working directory to Manuscript_data to run the vignette
...
ID89_ID83_connection_example <- data.table::fread("./89type_to_83type_connection.tsv")
colnames(ID89_ID83_connection_example)[1:3] <- c("ID89_signature","example_catalog","ID83_signature")
to.plot.WGS_PCAWG_HMF_indels.catalog <- data.table::fread("./Liu_et_al_83_type_spectra.tsv")
...
to.plot.all.signatures.final <- as.data.frame(data.table::fread("./Liu_et_al_final_83_type_signatures.tsv"))
...
to.plot.all.ID89.signatures.final <- as.data.frame(data.table::fread("./Liu_et_al_final_89_type_signatures.tsv"))
...
to.plot.all.ID89.catalogs <- as.data.frame(data.table::fread("./Liu_et_al_89_type_spectra.tsv"))
...
ID89.mSigAct.assignment <- as.data.frame(data.table::fread("./Liu_et_al_89_type_signature_assignments.tsv"))
row.names(ID89.mSigAct.assignment)<- ID89.mSigAct.assignment[,1]  # Use the first column as row names
ID89.mSigAct.assignment <- ID89.mSigAct.assignment[, -1]

final.Koh476.signatures <- as.data.frame(data.table::fread("./Liu_et_al_final_476_type_signatures.tsv"))
...
to.plot.all.ID476.catalogs <- as.data.frame(data.table::fread("./Liu_et_al_476_type_spectra.tsv"))
...
for (i in c(1:nrow(ID89_ID83_connection_example))) {
  ...
  if(ID89signature %in% c("InsDel15","InsDel16")){
    ...
    if(ID83signature %in% c("C_ID7","ID_J","ID_N","ID_O")){
      cat(paste0("\n\n### ", ID89signature, " \n\n  83-Type Signature: ", 
                 ID83signature,
    ...
    if(ID89signature %in% colnames(final.Koh476.signatures)){
      ...
      cosine476 <- round(lsa::cosine(as.numeric(final.Koh476.signatures[,ID89signature]),
                                    as.numeric(to.plot.all.ID476.catalogs[,catalog])),3)
      cat(paste0("\n\n Cosine similarity of 476-Type Signature vs Examplar: ",cosine476, "\n\n"))
      
      p5 <- PlotKoh476Catalog(final.Koh476.signatures[,ID89signature],text_size = 3,plot_title = paste0("476-Type Representation of ",ID89signature,sep=""),top.types.to.show = 5)
      p6 <- PlotKoh476Catalog(to.plot.all.ID476.catalogs[,catalog],text_size = 3,plot_title = paste0("476-Type Examplar Spectra of ",ID89signature,sep=""),top.types.to.show = 5)
      grid.arrange(p5,nrow = 1, ncol = 1)
    ...
    if(ID83signature %in% c("C_ID7","ID_J","C_ID10","ID_N","ID_O")){
      cosine83 <- round(lsa::cosine(as.numeric(to.plot.all.signatures.final[,ID89_ID83_connection_example$ID83_signature[i]]),
                                    as.numeric(to.plot.WGS_PCAWG_HMF_indels.catalog.no.polyT[,catalog])),3)
      cat(paste0("\n\n Cosine similarity of 83-Type Signature vs Examplar: ",cosine83, "\n\n"))
      ...
      cosine83 <- round(lsa::cosine(as.numeric(to.plot.all.signatures.final[,ID89_ID83_connection_example$ID83_signature[i]]),
                                    as.numeric(to.plot.WGS_PCAWG_HMF_indels.catalog[,catalog])),3)
      cat(paste0("\n\n Cosine similarity of 83-Type Signature vs Examplar: ",cosine83, "\n\n"))
  ...
  cat(paste0("\n\n### 83-type: ",cosine83,", 89-type: ", cosine1, ", 476-type:",cosine476,"\n\n"))
