...
cat1 <- ICAMS::annot_vcf_to_476_catalog(vcf, sample_id = "clip_le_9_TRUE",
                                         clip_le_9 = TRUE)

# Way #2: Pre-filter VCF to R <= 9, then use clip_le_9 = FALSE
vcf_filtered <- vcf[R <= 9, ]
cat2 <- ICAMS::annot_vcf_to_476_catalog(vcf_filtered, sample_id = "prefiltered_R_le_9",
                                         clip_le_9 = FALSE)
...
spectra <- read.delim("Manuscript_data/476_clipped_spectra.tsv",
                      check.names = FALSE)
